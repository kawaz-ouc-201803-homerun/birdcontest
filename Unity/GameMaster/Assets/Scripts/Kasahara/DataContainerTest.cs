	...
	void Start() {
		...
		switch(optionC) {
			...
			case 1: //応援
				...
				Cheerup.IsCheerupEnabled = true;

				//paramCをupperに代入
				this.GetComponent<Cheerup>().UpperPower = paramC;
		...
		switch(optionA) {
			...
			case 2:
				...
				if(Input.GetKeyDown(KeyCode.Return)) {
					...
					querychan.GetComponent<QueryChanAnimationController>().enabled = true;        //クエリちゃんのアニメーションを再生
					querychan.GetComponent<QueryChanAnimationController>().QueryChanAnimator.SetTrigger("Start");     //クエリちゃんが飛行機を押すアニメーションを再生
